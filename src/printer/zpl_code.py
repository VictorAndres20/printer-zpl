...
class ZplCoder:
    ...
    def build_label_code(self):
        code = (f"^XA^CI28^FX Third section with bar code.^BY2,3,100^FO60,10^BC^FD{self.model.id_barcode}^FS^FO50,150"
    ...
    def build_roler_code(self):
        code = (f"^XA"
                ...
                f"^FO50,250^FDMts: {self.model.label4}^FS^FO50,270^FD{self.model.label5}^FS^XZ")
        return code

    def build_nalsani_code(self):
        code = (f"^XA^CI28^FX Third section with bar code.^BY2,3,100^FO60,10^BC^FD{self.model.ean}^FS^FO50,150"
                f"^GB300,3,3^FS^FX Second section with recipient address and permit information."
                f"^CFA,14^FO50,170^FD{self.model.label1}^FS^FO50,190^FD{self.model.label8}^FS"
                f"^FS^FO50,210^FD{self.model.label2}^FS"
                f"^FO50,230^FD{self.model.label3}^FS^FO50,250^FDMts: {self.model.label4}^FS"
    ...
    def build_eliot_code(self):
        ...
        code += f"^BY2,3,100^FO60,60^BC^FD1{self.model.ean}3{build_eliot_barcode(self.model.label4)}.000:    ^FS^XZ"
        return code
